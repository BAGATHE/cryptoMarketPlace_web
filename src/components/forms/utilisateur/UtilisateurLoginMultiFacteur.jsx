...
import { loginUtilisateur, validateOtp, loginUtilisateurCrypto } from "../../../services/utilisateur.service";
import Swal from "sweetalert2";
import { synchroniseLogin } from '../../../services/synchronisation.service';

const UtilisateurLoginMultiFacteur = () => {
  ...
  const onFinishLogin = async (values) => {
    ...
    try {
      ...
      const data = await loginUtilisateur(payload);
  ...
  const onFinishOtp = async (values) => {
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        const dataUtilisateur = await loginUtilisateurCrypto(utilisateur);
...
export default UtilisateurLoginMultiFacteur;
